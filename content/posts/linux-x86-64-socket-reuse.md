...
ซึ่งการทำ Socket Reuse คือการที่เราจะใช้ FD ที่เราใช้ติดต่อกับ socket server ซ้ำเพื่อช่วยในการโจมตี ต่อไป ที่จะทำในวันนี้คือเราจะทำการเรียก shell แล้วก็ทำการติดต่อกับ shell ของเราโดยใช้ FD ที่เราทำการเชื่อมต่อกับ socket server
...
หากว่า sockfd ที่เราใส่เข้าไปในฟังก์ชัน getpeername นั้นเป็น FD ของ socket จริง ๆ ค่าที่ฟังก์ชัน return กลับมาจะเป็น 0 
...
เราทำการ `push rdx` ซึ่งมีค่าเป็น 0 ลงไปใน stack 2 ครั้งบน Intel x86-64 การ push แต่ละครั้งจะทำการลบค่า rsp ไป 8 (`rsp = rsp - 8`)
จากนั้นทำการย้ายค่าของ rsp เข้าไปเก็บที่ rsi 
```nasm
	; rsi = *addr
...
*สำหรับบางคนที่พึ่งหัดเขียน Assembly เพื่อไปเป็น shellcode อาจเริ่มจากเขียนให้ถูกก่อน จากนั้นค่อยมาทำการแก้ไขคำสั่งเพื่อไม่ให้มี null byte และมีขนาดสั้นลงในภายหลัง แต่คนเท่ ๆ แบบผมเก่งอยู่แล้ว เขียนไป optimize ไปได้เลย 😎(หยอก ๆ)*

พอ ๆ โม้มาเยอะและ กลับเข้ามา shellcode ของเราดีกว่าค้าบ
ตอนนี้ argument สำหรับฟังก์ชัน getpeername เราพร้อมเรียบร้อยแล้ว

1.2 ทำการเรียก getpeername ผ่าน Syscall 

จะเห็นว่าเรามีการทำ label ชื่อ loop_findfd เพื่อใช้ในการ jmp กลับมาเพื่อทำการ loop การทำงาน จากนั้นทำการเพิ่มค่า rdi ขึ้น 1 (ตอนแรกมีค่าเป็น 0) นั่นหมายความว่าค่า rsi ซึ่งเป็นตัวแปร sockfd ที่เราส่งค่าเข้าไปในฟังก์ชัน getpeername จะเพิ่มขึ้นทุกครั้งเมื่อทำการ loop นั่นเอง
```nasm
; syscall getpeername
loop_findfd:
    ; rdi = sockfc
...
จากนัั้นก็ทำลักษณะตั้งค่า rax เป็น 0x34(จะใส่เป็นเลขฐาน 10 หรือ 16 ก็ได้แล้วแต่ถนัด) ลักษณะเดียวกันกับก่อนหน้านั่นเอง
```nasm
    xor rax, rax        ; rax = 0
...
โดยปกติแล้วบน Intel x86-64 เวลาฟังก์ชัน Return ค่าจะส่งผ่าน rax ซึ่ง syscall ของ getpeername ก็เช่นเดียวกัน

อย่างที่เราได้อธิบายไว้ตอนอ่าน man page ว่าหากเจอ FD ที่เป็น sockfd ค่าที่ return กลับมาจะต้องเป็น 0 (rax = 0) นั่นเอง
...
เมื่อเราได้ค่า sockfd ที่เราต้องการ(ซึ่งตอนนี้อยู่ที่ rdi) สิ่งที่เราต้องทำต่อไปคือใช้งานฟังก์ชัน dup2 เพื่อให้เชื่อม sockfd ของ socket เรากับ STDIN, STDOUT และ STDERR จะทำให้เราสามารถติดต่อกับ shell ได้นั้นเอง

ซึ่งจาก [man](https://man7.org/linux/man-pages/man2/dup2.2.html) page dup2 ต้องการ argument ดังนี้
...
จะเห็นว่าเราต้องทำการเรียก dup2 ทั้งหมด 3 รอบโดยค่า rdi คงเดิมและค่า rsi เพิ่มขึ้นทีละ 1

จากข้อก่อนหน้าค่าของ rdi เป็น sockfd อยู่แล้วนั่นหมายความว่าเราไม่ต้องไปแก้ไขอะไรเลย
